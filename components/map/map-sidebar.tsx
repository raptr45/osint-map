...
export function MapSidebar({ events, isLoading, fetchError, mapRef }: MapSidebarProps) {
  const {
    isSidebarOpen,
  ...
  const sidebarEvents = React.useMemo(() => {
    let evts = events;
    if (clusterFilter) evts = evts.filter((e) => clusterFilter.includes(e.id));
    if (eventTypeFilter) {
      evts = evts.filter((e) => {
        const type = e.eventType && e.eventType !== "unknown" ? e.eventType : deriveEventType(e.title);
  ...
  const theaterStats = React.useMemo(() => {
    ...
    const stats = { total: 0, critical: 0, types: {} as Record<string, number> };
    events.forEach((e) => {
      stats.total++;
      ...
      <div className="p-6 flex flex-col h-full">
        ...
        {clusterFilter && (
          <div className="flex items-center justify-between gap-2 mb-3 px-3 py-2 rounded-xl bg-orange-500/10 border border-orange-500/20 text-orange-400">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold uppercase tracking-widest">{clusterFilter.length} events in cluster</span>
            </div>
            <button onClick={() => setClusterFilter(null)} className="hover:text-orange-300 transition-colors" title="Clear filter">
        ...
        {eventTypeFilter && !theaterStats && !clusterFilter && (
          <div className="flex items-center justify-between gap-2 mb-3 px-3 py-2 rounded-xl bg-primary/10 border border-primary/20 text-primary">
            <div className="flex items-center gap-2">
              <Filter className="w-3.5 h-3.5 shrink-0" />
              <span className="text-[11px] font-bold uppercase tracking-widest">Filtered: {eventTypeFilter.replace("_", " ")}</span>
            </div>
            <button onClick={() => setEventTypeFilter(null)} className="hover:text-primary/70 transition-colors" title="Clear filter">
              <X className="w-3.5 h-3.5" />
            </button>
          </div>
        )}
        ...
        {theaterStats && !clusterFilter && (
          <div className="mb-4 space-y-2 border-b border-border/20 pb-4">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground font-display">Theater Overview</span>
              ...
              {Object.entries(EVENT_TYPE_LABELS).map(([type, { label }]) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[10px] font-bold bg-background/50 px-1.5 py-0.5 rounded-full mt-1.5 min-w-[20px]">{count}</span>
                  </button>
                );
              })}
            </div>
            ...
            Object.entries(groupedEvents).map(([date, dayEvents]) => (
              <div key={date} className="space-y-3">
                <div className="sticky top-0 bg-background/40 backdrop-blur-md py-1 z-10 flex items-center gap-2">
                  <span className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground/60">{date}</span>
                ...
                {dayEvents.map((e) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4 mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={(ev) => { ev.stopPropagation(); mapRef.current?.panTo([e.lng, e.lat]); }}
                          title="Locate on map"
                          className="opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground/50 hover:text-primary"
                        >
                          <Maximize2 className="w-3 h-3" />
                        </button>
                        <span className="text-[11px] font-medium text-muted-foreground tabular-nums flex-shrink-0">{formatTime(e.createdAt)}</span>
                      </div>
                    </div>
                    <p className="text-xs text-muted-foreground/80 line-clamp-2 leading-relaxed mb-3">{e.description}</p>
                    <div className="flex items-center justify-between">
                      {e.sourceUrl ? (
                        <a
                          href={e.sourceUrl} target="_blank" rel="noopener noreferrer"
