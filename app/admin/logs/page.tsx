...
export default function SystemLogsPage() {
  ...
  if (isLoading) {
    return (
      <div className="p-4 sm:p-8 space-y-8 flex flex-col items-center justify-center min-h-[60vh] opacity-50 px-6 text-center">
        <Network className="w-10 h-10 animate-pulse text-primary" />
        <p className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-primary italic">Synchronizing audit trail...</p>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-8 md:p-10 space-y-8 sm:space-y-12 animate-in fade-in duration-700 font-sans max-w-7xl mx-auto overflow-x-hidden">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2 border-b border-white/5">
        ...
        <div className="flex items-center gap-3 justify-center sm:justify-end">
           <Button
            ...
            className="tactical-btn-outline h-10 sm:h-11 px-4 sm:px-6"
          >
          ...
          <Button
            ...
            className="tactical-btn h-10 sm:h-11 px-4 sm:px-6 shadow-2xl shadow-primary/20"
          >
            <RefreshCcw className="w-4 h-4 sm:mr-2" /> <span className="hidden sm:inline">Refresh</span>
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
        {[
          ...
          <div key={i} className="tactical-card group p-5 sm:p-7 cursor-default">
            <div className="premium-glow" />
            <div className="flex items-center gap-4 sm:gap-6 relative z-10">
              <div className={cn(
                "w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center border shadow-2xl transition-transform duration-500 group-hover:scale-110",
              ...
              <div className="flex-1 space-y-0.5 sm:space-y-1">
                <p className="text-[9px] sm:text-[11px] font-black text-muted-foreground/50 uppercase tracking-[0.2em] leading-none">
                  {stat.label}
                </p>
                <p className="text-xl sm:text-2xl font-black font-display text-white group-hover:text-primary transition-colors tracking-tight italic">{stat.value}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="tactical-card group rounded-[2.5rem] flex flex-col min-h-[60vh] overflow-hidden">
        ...
        <div className="flex-1 overflow-x-auto relative z-10">
          {!isMobile ? (
            <table className="w-full text-left border-collapse min-w-max sm:min-w-0">
              <thead>
            ...
            <div className="divide-y divide-white/5">
              {filteredLogs.map((log) => (
                <div key={log.id} className="p-6 space-y-4 hover:bg-primary/5 transition-all">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                       ...
                       <span className={cn("text-[9px] font-black uppercase tracking-widest", `text-${getLevelColor(log.level)}-500`)}>{log.level}</span>
                    </div>
                    <span className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-widest">{formatDistanceToNow(new Date(log.createdAt))}</span>
                  </div>
                  <p className="text-xs font-bold text-white/90 leading-relaxed" onClick={() => handleCopy(log.message, log.id)}>{log.message}</p>
                  <div className="flex items-center justify-between pt-2">
                    <Badge variant="secondary" className="text-[8px] px-2 py-0.5 bg-white/5 border-white/5 text-muted-foreground/60 uppercase font-black">{log.module}</Badge>
                    <button onClick={() => handleCopy(log.id, log.id)} className="text-[8px] font-mono text-muted-foreground/40 flex items-center gap-2 italic">#{log.id.substring(0, 8)} <Copy className="w-2.5 h-2.5" /></button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {filteredLogs.length === 0 && (
            <div className="py-20 text-center text-muted-foreground italic text-xs uppercase tracking-widest opacity-20 relative z-10">Waiting for system telemetry...</div>
          )}
        </div>

        <div className="p-6 sm:p-8 bg-primary/5 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-6 relative z-10">
          <span className="text-[9px] sm:text-[10px] font-black text-muted-foreground/30 uppercase tracking-[0.2em] sm:tracking-[0.3em] text-center sm:text-left italic">
            SYSTEM CLOCK SYNCED: {new Date().toISOString().split('T')[1].split('.')[0]} UTC
          </span>
          <div className="flex items-center gap-4 sm:gap-8 w-full sm:w-auto justify-between sm:justify-end">
            <button
               onClick={() => setIsPaused(!isPaused)}
               className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest flex items-center gap-2 sm:gap-3 text-muted-foreground hover:text-white transition-all group/btn"
            >
              <div className={cn("w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-2xl flex items-center justify-center transition-all", isPaused ? "bg-amber-500/10 text-amber-500" : "bg-white/5")}>
                 {isPaused ? <Play className="w-3.5 h-3.5 animate-pulse" /> : <Pause className="w-3.5 h-3.5 group-hover/btn:scale-110" />}
              </div>
              <span className="hidden sm:inline">{isPaused ? "RESUME STREAM" : "PAUSE STREAM"}</span>
              <span className="sm:hidden">{isPaused ? "RESUME" : "PAUSE"}</span>
            </button>
            
            <div className="flex items-center gap-2 sm:gap-3 px-4 py-2 sm:px-6 sm:py-3 rounded-lg sm:rounded-[1.5rem] bg-white/5 border border-white/5 shadow-2xl">
               <div className={cn("w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full border-2 border-background", isPaused ? "bg-amber-500" : "bg-emerald-500 animate-pulse shadow-[0_0_12px_rgba(16,185,129,0.5)]")} />
               <span className={cn("text-[10px] sm:text-[11px] font-black uppercase tracking-widest", isPaused ? "text-amber-500" : "text-emerald-500")}>
